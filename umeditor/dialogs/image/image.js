...
//                }
    ...
    var Upload = {
        ...
        uploadTpl: '<div class="edui-image-upload%%">' +
            ...
            '<form id="ueditorForm" file-model="myFile" class="edui-image-form" method="post" enctype="multipart/form-data"  target="up">' +
            '<input id="InputImg" style=\"filter: alpha(opacity=0);\" class="edui-image-file" type="file" hidefocus name="%fieldName%" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp"/>' +
            '</form>' +
            '</div>',
        ...
        config: function (sel) {
            var me = this,
      		...
      		url=url + (url.indexOf("?") == -1 ? "?" : "&")//初始form提交地址;
//          url=url + (url.indexOf("?") == -1 ? "?" : "&") + "editorid="+me.editor.id;//初始form提交地址;
       ...
       //      	$('iframe').on('load', function(){
             $('<iframe name="up"   id="frame"><div id="iframeDiv" ></div></iframe>').insertBefore(me.dialog).on('load', function(){
