...
app.controller("codeOldPhoneC", function($scope, $window, $interval) {
	...
	$scope.sendCode = function() {
		...
		//end 验证码倒计时
	var newPhone = location.href.split("=")[1];
	$scope.next = function() {

		if($scope.register_code == null || $scope.register_code.length == 0) {
			$scope.codeShow = true;
			return;
		}
		...
		$.ajax({
			...
			.done(function(data) {
				if(data.flag == 1) {
					Toast("修改成功~~");
					$window.location.href = "modifySuccess.html?phone=" + newPhone;
				} else {
					$scope.$apply(function() {
						$scope.codeShow = true;
					})
