...
app.controller("modifyPhoneC", function($scope, $window) {
	...
	if(userInfo.email) {
		$scope.showEmailNow = true;
	} else {
		$scope.showEmailNow = false;
	}
	...
	$scope.next = function() {
		if($scope.nickName == undefined || $scope.nickName.trim() == "" || $scope.nickName.length == 0) {
			$scope.isNull = true;
			return;
		}
		if(!$scope.nickName.match(/^(\w-*\.*)+@(\w-?)+(\.\w{2,})+$/)) {
			$scope.change = true;
			return;
		}
				...
				if(data.flag == 1) {
					$scope.$apply(function() {
						if(data.objectList == false) {
							$scope.ishave = false;
							sessionStorage.setItem("fixemailverify", $scope.nickName)
							$window.open("codeNewEmail.html", "_self")
							return;
						}
