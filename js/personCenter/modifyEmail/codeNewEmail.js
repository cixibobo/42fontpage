...
app.controller("codeNewC", function($scope, $window, $interval) {
	...
	if(!sessionStorage.getItem("fixemailverify")) {
		Toast("获取邮箱失败");
		return;
	} else {
		var email = sessionStorage.getItem("fixemailverify");
		$scope.email = email;
	}
			...
			.done(function(data) {
				if(data.flag == 1) {
					...
					if(!userInfo.email) {
								...
								if(data.flag == 1) {
									$.ajax({
										...
										.done(function(data) {
											...
											$window.location.href = "modifySuccess.html?email=" + email;
										})
						...
						$window.location.href = "codeOldEmail.html?email=" + email;
					}
				} else {
					$scope.$apply(function() {
						$scope.codeShow = true;
