var app = angular.module("getPraiseApp", ['tm.pagination']);
app.controller("getPraiseC", function($scope, $window) {
	if(!isLogin()) {
		Toast("请先登陆", 1000);
	...
	$scope.changFrameTop = function(frame_src) {
		...
		//请求赞我的评论
	$scope.queryPraise = function() {

		$.ajax({
				url: IP + '/messageCenter/selectCommentpraise.do',
				type: 'POST',
				dataType: 'json',
				async: true,
				data: {
